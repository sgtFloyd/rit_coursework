...
end_game:
	...
	li		$v0, PRINT_STRING
	la		$a0, p2_win
	syscall							# print player 2's win message
	j		exit_program
win_p1:
	jal		print_board
	li		$v0, PRINT_STRING
	la		$a0, p1_win
	syscall							# print player 1's win message
	j		exit_program
tie_game:
	jal		print_board
	li		$v0, PRINT_STRING
...
# If the move is valid, it returns the move in $v0. If the move isn't valid, it
...
get_input_loop:
	li		$v0, PRINT_STRING
	beq		$s7, $zero, p1_in		# find who's turn it is
	la		$a0, p2_msg
	j		end_input_msg
p1_in:
	la		$a0, p1_msg
...
print_error:
	...
	j		get_input_loop
good_input:
	move	$v0, $t0				# return player's legal move
	lw    	$ra, 0($sp)         	# restore $ra
	addi  	$sp, $sp, 4
	jr		$ra						# return
...
force_quit:
	...
	la		$a0, p2_quit			# prepare player 2's quit message
	j		end_exit
p1_exit:
	la		$a0, p1_quit			# prepare player 1's quit message
end_exit:
...
# Name:		make_move
#
# Description:	Takes the player's input in $a0 and drops a piece in that
...
cell_empty:
	...
	addi	$t5, $zero, 2			# drop player 2's piece
	j		end_drop
drop_X:
	addi	$t5, $zero, 1			# drop player 1's piece
end_drop:
...
hor_o_loop:
	beq		$t9, $t4, end_hor 		# if we're off the board, break
...
check_board_loop:
	...
	beq		$t5, $zero, not_full 	# if t5 = 0, board isn't full
...
toggle_player:
	beq		$s7, $zero, make_p2
	addi	$s7, $s7, -1			# if it's p2's turn, make it p1's
	j		end_toggle
make_p2:
	addi	$s7, $s7, 1				# if it's p1's turn, make it p2's
end_toggle:
	jr		$ra						# return
